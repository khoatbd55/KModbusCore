...
{
    public class KCrossPlatformSocket : IDisposable
    {
        ...
        public KCrossPlatformSocket(AddressFamily addressFamily)
        {
            _socket = new Socket(addressFamily, SocketType.Stream, ProtocolType.Tcp);
            _socketDisposeAction = _socket.Dispose;
        }

        public KCrossPlatformSocket()
        {
        ...
        public KCrossPlatformSocket(Socket socket)
        {
        ...
        public async Task<KCrossPlatformSocket> AcceptAsync()
        {
            try
            {
#if NET452 || NET461
...
#endif
                return new KCrossPlatformSocket(clientSocket);
            }
        ...
        public async Task ConnectAsync(string host, int port, CancellationToken cancellationToken)
        {
            ...
            catch (SocketException socketException)
            {
                ...
                {
                    throw new ModbusCommunicationTimedOutException();
                }

                throw new ModbusCommunicationException($"Error while connecting with host '{host}:{port}'.", socketException);
            }
            catch (ObjectDisposedException)
            {
